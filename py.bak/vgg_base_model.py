...
import utils
from models import VGG_base_model
...
def main():
    ...
    vgg_cloud_model_optimizer = optim.SGD(vgg_base_model.parameters(),
                                          lr=cfg['lr'],
    ...
    if os.path.isdir('./figure') == False:
        os.mkdir('./figure')
    plt.tight_layout()
    plt.savefig('./figure/vgg_base_model.png')

    if os.path.isdir('./checkpoint') == False:
        os.mkdir('./checkpoint')
    torch.save(vgg_base_model.state_dict(), './checkpoint/vgg_base_model.pth')

    vgg_base_model_result = {
        'epochs': epochs,
        'accs': accs,
    ...
    if os.path.isdir('./result') == False:
        os.mkdir('./result')
    with open('./result/vgg_base_model.pkl', 'wb') as f:
        pickle.dump(vgg_base_model_result, f)
