...
def main():
    ...
    co_control_model_optimizer = optim.SGD(co_control_model.parameters(),
                                           lr=cfg['lr'],
    ...
    def train(current, iterations, accs):
        ...
        for batch_idx, (x, label) in enumerate(trainloader):
            ...
            co_control_model_optimizer.zero_grad()

            x = shared_encoder(x)
            y = co_control_model(x)
            y_ = control_model(x)
            # y += y_
            y = y.clone() + y_
            ...
            co_control_model_optimizer.step()
    ...
    torch.save(co_control_model.state_dict(),
               './checkpoint/co_control_model.pth')
